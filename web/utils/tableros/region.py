...
df_municipios = pd.read_excel(data_folder+'/region.xlsx',sheet_name='municipios_mat')
...
df_dpto_mat_privado = pd.read_excel(data_folder+'/region.xlsx',sheet_name='dpto_mat_privado')
df_dpto_mat_oficial = pd.read_excel(data_folder+'/region.xlsx',sheet_name='dpto_mat_oficial')
df_dpto_mat_hombres = pd.read_excel(data_folder+'/region.xlsx',sheet_name='dpto_mat_hombres')
df_dpto_mat_mujeres = pd.read_excel(data_folder+'/region.xlsx',sheet_name='dpto_mat_mujeres')
df_dpto_grad = pd.read_excel(data_folder+'/region.xlsx',sheet_name='dpto_graduados')
...
def mapa(dpto,periodo):
    try:
            ...
            if loc['properties']['NOMBRE_DPT'] == dpto:
                ...
                for c in coords:
                    try:
                        ...
                        pass
                logitud, latitude = statistics.mean(longitudes), statistics.mean(latitudes)
        ...
        municipios = municipios[['municipio','latitud','longitud',str(periodo)]]
        ...
        municipios['text'] = [ '<b>{}</b><br>{}'.format(nom,mat) for nom,mat in zip(municipios['municipio'],municipios[str(periodo)])]
...
def barras(dpto):
    try:
        df = df_dpto_des[df_dpto_des['departamento']==dpto].drop(['departamento'],axis=1)
...
def pastel(dpto, periodo):
    try:
        matricula_total = df_dpto_mat[df_dpto_mat['departamento']==dpto][str(periodo)].values[0]
        oficial = df_dpto_mat_oficial[df_dpto_mat_oficial['departamento']==dpto][str(periodo)].values[0]
        privado = df_dpto_mat_privado[df_dpto_mat_privado['departamento']==dpto][str(periodo)].values[0]
        ...
        fig.update_traces(
            hovertext= ['<b>{}:</b> <br>{}'.format(t,c) for t,c in zip(df['tipo'],df['cantidad'])],
...
def genero(dpto,periodo):
    try:
        matricula_total = df_dpto_mat[df_dpto_mat['departamento']==dpto][str(periodo)].values[0]
        hombres = df_dpto_mat_hombres[df_dpto_mat_hombres['departamento']==dpto][str(periodo)].values[0]
        mujeres = df_dpto_mat_mujeres[df_dpto_mat_mujeres['departamento']==dpto][str(periodo)].values[0]
...
def indicadores_dpto(dpto,periodo):
    try:
        ...
        fig = make_subplots(
            column_titles=['Deserción', 'Cobertura', 'Matriculados', 'Graduados'],
        ...
        for i,df_var_ind in enumerate(variables_indicadores):
            periodo_actual = df_var_ind[str(periodo)].values[0]*100
            periodo_anterior = df_var_ind[str(periodo-1)].values[0]*100 if str(periodo-1) in periodos else None
...
def indicadores_ies(dpto):
    try:
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "scatter",'l':0,'r':0,'t':0,'b':0}, {"type": "indicator",'l':0,'r':0,'t':0,'b':0}]]*cant_ies,
        )
