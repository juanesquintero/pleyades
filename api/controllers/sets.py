...
from db.pleyades.db import DataSet as set_model, Execution as execution_model, Preparation as preparation_model
...
def name():
    ...
    # Definir el name del student_set con la notacion
    name = nombre_corto+' ' + \
...
def delete_many(estado):
    ...
    sets_nombres = [c['name'] for c in query]
...
def delete_one(name):
    ...
    # delete
    delete = set_model.delete(name)
    # delete results
    delete = execution_model.delete_set(name)
    delete = preparation_model.delete_set(name)
    ex = exception(delete)
    if ex:
        return ex
...
def put(name):
    ...
    # Uptade
    update = set_model.update(name, body)
    ex = exception(update)
    if ex:
        return ex
